 ...
 private:
  ...
  bool parseAnd(int16_t& index, std::vector<lamp::live::Diagnostic>& diagnostics) {
    ...
    while (true) {
      ...
      position_ += 2;
      int16_t rhs = -1;
      if (!parseComparison(rhs, diagnostics)) return false;
      ExpressionNode node;
  ...
  bool parseComparison(int16_t& index, std::vector<lamp::live::Diagnostic>& diagnostics) {
    ...
    while (true) {
      ...
      if (position_ + 1 < input_.size()) {
        if (input_[position_] == '>' && input_[position_ + 1] == '=') { cmpOp = ExpressionOp::kGte; position_ += 2; }
        else if (input_[position_] == '<' && input_[position_ + 1] == '=') { cmpOp = ExpressionOp::kLte; position_ += 2; }
        else if (input_[position_] == '=' && input_[position_ + 1] == '=') { cmpOp = ExpressionOp::kEq; position_ += 2; }
        else if (input_[position_] == '!' && input_[position_ + 1] == '=') { cmpOp = ExpressionOp::kNeq; position_ += 2; }
        else if (input_[position_] == '>') { cmpOp = ExpressionOp::kGt; position_ += 1; }
        else if (input_[position_] == '<') { cmpOp = ExpressionOp::kLt; position_ += 1; }
        else { return true; }
      } else {
        if (input_[position_] == '>') { cmpOp = ExpressionOp::kGt; position_ += 1; }
        else if (input_[position_] == '<') { cmpOp = ExpressionOp::kLt; position_ += 1; }
        else { return true; }
      }
      int16_t rhs = -1;
  ...
  bool parseUnary(int16_t& index, std::vector<lamp::live::Diagnostic>& diagnostics) {
    ...
    if (match('!')) {
      int16_t child = -1;
      if (!parseUnary(child, diagnostics)) return false;
      ExpressionNode node;
