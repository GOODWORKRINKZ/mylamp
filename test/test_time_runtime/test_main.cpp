...
class FakeTimeSource : public lamp::time::ITimeSource {
 public:
  ...
  bool syncTime(const char* timezone, const char* primaryServer,
    ...
    syncCalled = true;
    timezoneValue = timezone ? timezone : "";
    if (syncResult) {
      cachedValid = true;
    }
    return syncResult;
...
void test_runtime_service_syncs_time_when_ntp_is_enabled() {
  ...
  lamp::time::ClockSettings settings;
  settings.timezone = "MSK-3";
  ...
  const lamp::time::RuntimeTimeState state = service.refresh(settings, plan, source);

  TEST_ASSERT_TRUE(source.syncCalled);
  TEST_ASSERT_EQUAL_STRING("MSK-3", source.timezoneValue.c_str());
...
void test_runtime_service_uses_cached_time_offline() {
  ...
  lamp::time::ClockSettings settings;
  settings.timezone = "CET-1CEST,M3.5.0,M10.5.0/3";
  lamp::time::PlannedTimeState plan;
  ...
  TEST_ASSERT_TRUE(source.syncCalled);
  TEST_ASSERT_EQUAL_STRING("CET-1CEST,M3.5.0,M10.5.0/3", source.timezoneValue.c_str());
  TEST_ASSERT_TRUE(state.hasValidTime);
